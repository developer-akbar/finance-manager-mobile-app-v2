...
export const replaceBudgets = async (items) => {
  ...
  for (const item of items) {
    ...
    const prd = item.period || 'Monthly';
    const catTime = item.created_at || new Date().toISOString();
    await db.run('INSERT INTO budgets (id,category,amount,period,created_at) VALUES (?,?,?,?,?)', [id, cat, amt, prd, catTime]);
  }
};
