...
const matchedCasIndices = new Set();
...
buyCasRows.forEach((cas, idx) => {
  ...
  for (let i = 0; i < baseFinManRows.length; i++) {
    ...
    matchedCasIndices.add(idx);
    break;
  }
});

console.log(`Matched BUYs count: ${matchedCasIndices.size} / ${buyCasRows.length}`);

const unmatchedBUYs = buyCasRows.filter((_, idx) => !matchedCasIndices.has(idx));
console.log(`Unmatched BUYs count: ${unmatchedBUYs.length}`);
unmatchedBUYs.forEach((u, i) => {
  console.log(`[Unmatched BUY ${i+1}] Date: ${u.Date} | Scheme: ${u.SecuritySymbol} | Units: ${u.Quantity} | Val: ₹${u.TradeValue}`);
});
