...
function parseTxnFields(t) {
  ...
  if (type) {
    ...
    return {
      ...
      cashImpact: isRecon ? 0 : parseFloat(t.CashImpact || 0),
...
function calculateBrokerageState(txns, brokerConfigList = [], settings = {}) {
  // Identify brokerages dynamically
...
console.log('=== ZERODHA STATE ===');
console.log(JSON.stringify(state.Zerodha, null, 2));

console.log('=== FAREEDA GROWW STATE ===');
console.log(JSON.stringify(state['Fareeda Groww'], null, 2));
