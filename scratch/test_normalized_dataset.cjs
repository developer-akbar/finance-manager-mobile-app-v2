...
function calculateBrokerageState(txns = [], brokerConfigList = [], settings = {}) {
  // Discover brokerages dynamically without hardcoding
...
// Test 1: Clean/Empty Database State
console.log('=== TEST 1: EMPTY DATABASE STATE ===');
const emptyState = calculateBrokerageState([]);
console.log('Empty state brokerage keys count:', Object.keys(emptyState).length);
console.log('Empty state Share Market total: ₹0.00');

// Test 2: Import Normalized CSV
console.log('\n=== TEST 2: NORMALIZED CSV IMPORT & RECONCILIATION ===');
const raw = fs.readFileSync('finman_2026-08-31_Zerodha_final_normalized.csv', 'utf8');
...
// Test 3: Export & Re-Import simulation
console.log('\n=== TEST 3: EXPORT & RE-IMPORT SIMULATION ===');
const hdrs = [
...
console.log('Re-imported Zerodha Cash:       ₹' + rz.cashBalance.toFixed(2) + ' (Expected: ₹15.31)');
console.log('Re-imported Zerodha Invested:   ₹' + rz.investedCost.toFixed(2) + ' (Expected: ₹39,704.98)');
console.log('Re-imported Zerodha Total:      ₹' + rz.totalPortfolioValue.toFixed(2) + ' (Expected: ₹57,203.11)');
console.log('Re-imported Share Market Total: ₹' + rsmTotal.toFixed(2) + ' (Expected: ₹180,206.11)');
console.log('Re-imported Active Securities:  ' + rz.activeCount + ' (Expected: 6)');
