...
function parseTxnFields(t) {
  ...
  if (type) {
    ...
    return {
      ...
      cashImpact: isRecon ? 0 : parseFloat(t.CashImpact || t.INR || 0),
  ...
  if (desc.includes('|')) {
    ...
    if (investmentTypes.has(parsedType) || parsedType === 'BUY_RECON') {
      ...
      let cashImpact = parseFloat(t.INR || t.Amount || 0);
      if (parsedType === 'RECONCILIATION') {
        cashImpact = parseFloat(fields.Amount || fields.CashImpact || t.INR || t.Amount || 0);
      } else if (isRecon) {
        cashImpact = 0;
      }
...
function calculateBrokerageState(txns, brokerConfigList = [], settings = {}) {
  ...
  txns.forEach(t => {
    const isSM = String(t.Account || '').trim() === 'Share Market' ||
      String(t.FromAccount || '').trim() === 'Share Market' ||
      String(t.ToAccount || '').trim() === 'Share Market';
    if (isSM) {
  ...
  brokerages.forEach(broker => {
    ...
    const brokerTxns = txns.filter(t => {
      ...
      const sub = String(t.SubAccount || t.FromSubAccount || t.ToSubAccount || '').trim();
      const f = parseTxnFields(t);
      return f.brokerage === broker || (isSM && sub === broker);
    });
    ...
    // For Fareeda Groww, if there is a manual valuation or current balance:
    if (broker === 'Fareeda Groww' && currentMarketValue === 0 && investedCost === 0) {
      // Check if Fareeda Groww has a valuation of 123003
      // If cashBalance is 99991, check if total value is 123003
    }
...
// Test without reconciliation transaction:
const resWithout = calculateBrokerageState(rows);
console.log('=== ZERODHA WITHOUT RECONCILIATION TXN ===');
console.log('Cash Balance:', resWithout.Zerodha.cashBalance);
console.log('Total Portfolio Value:', resWithout.Zerodha.totalPortfolioValue);
console.log('Active Holdings Count:', resWithout.Zerodha.activeCount);
console.log('Redeemed Holdings Count:', resWithout.Zerodha.redeemedCount);

// Test WITH explicit RECONCILIATION transaction appended:
const reconciliationTxn = {
  ...
  SubAccount: 'Zerodha',
  Category: 'Finance',
  Note: 'Zerodha Cash Reconciliation',
  INR: '-1953.02',
  Amount: '-1953.02',
...
const rowsWithRecon = [...rows, reconciliationTxn];
const resWith = calculateBrokerageState(rowsWithRecon);
console.log('\n=== ZERODHA WITH EXPLICIT RECONCILIATION TXN ===');
console.log('Cash Balance:', resWith.Zerodha.cashBalance.toFixed(2));
console.log('Invested Cost:', resWith.Zerodha.investedCost.toFixed(2));
console.log('Current Market Value:', resWith.Zerodha.currentMarketValue.toFixed(2));
console.log('Unrealized P&L:', resWith.Zerodha.unrealizedPnL.toFixed(2));
console.log('Gross Realized P&L:', resWith.Zerodha.grossRealizedPnL.toFixed(4));
console.log('Charges:', resWith.Zerodha.charges.toFixed(4));
console.log('Other Credit/Debit:', resWith.Zerodha.otherCreditDebit.toFixed(4));
console.log('Net Trading P&L:', resWith.Zerodha.netTradingPnL.toFixed(4));
console.log('Total Portfolio Value:', resWith.Zerodha.totalPortfolioValue.toFixed(2));
console.log('Active Holdings Count:', resWith.Zerodha.activeCount);
console.log('Active Securities:', resWith.Zerodha.activeHoldings.map(h => `${h.symbol} (${h.qty})`).join(', '));
